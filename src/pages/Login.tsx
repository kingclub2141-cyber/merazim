...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { role, error } = await signIn(identifier, password);
    ...
    <div className="min-h-screen bg-[#fafafa] flex items-center justify-center p-6 selection:bg-[#E13D4B] selection:text-white">
      <div className="w-full max-w-[440px] space-y-8">
        ...
        <div className="text-center space-y-4">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-[#E13D4B] rounded-[24px] shadow-2xl shadow-gray-200 mb-2">
            <Dumbbell className="text-white" size={32} />
          </div>
          <h1 className="text-4xl font-black text-[#E13D4B] tracking-tighter uppercase italic">Mera Gym<span className="text-[#E13D4B]">.</span></h1>
        ...
        <div className="bg-white rounded-[40px] shadow-2xl shadow-gray-200/50 border border-gray-100 p-10 relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-6 relative z-10">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-5 bg-[#E13D4B] text-white font-black rounded-2xl text-[11px] uppercase tracking-[0.2em] shadow-2xl shadow-[#E13D4B]/20 hover:bg-[#E13D4B]/90 active:scale-[0.98] transition-all flex items-center justify-center gap-3 group"
            >
              {loading ? (
