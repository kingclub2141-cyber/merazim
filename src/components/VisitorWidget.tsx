...
export default function VisitorWidget() {
  ...
  return (
    <div className="bg-white border-8 border-[#E13D4B] shadow-[16px_16px_0px_0px_rgba(225,61,75,1)] overflow-hidden flex flex-col h-full">
      <div className="p-6 bg-[#E13D4B] text-white flex items-center justify-between">
         ...
         <button 
           ...
           className="bg-white text-[#E13D4B] px-3 py-1 text-[9px] font-black uppercase border-2 border-white hover:bg-transparent hover:text-white transition-all shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] active:shadow-none"
         >
            Fast Entry
         </button>
      </div>

      <div className="p-8 grid grid-cols-2 gap-8 border-b-4 border-[#E13D4B]/5">
      ...
      <div className="flex-1 overflow-y-auto">
         {loading ? (
            ...
            <div className="divide-y-2 divide-[#E13D4B]/5 px-2">
               {stats.recent.map((v: any) => (
                  <button 
                    key={v.id}
                    onClick={() => navigate(`/visitors/${v.id}`)}
                    className="w-full p-4 flex items-center justify-between hover:bg-rose-50 transition-all group text-left"
                  >
                     <div className="flex items-center gap-4">
                        <div className="w-8 h-8 bg-[#E13D4B] text-white flex items-center justify-center font-black italic text-xs border border-[#E13D4B]">
                           {v.name.charAt(0)}
                        </div>
      ...
      <button 
        ...
        className="p-4 bg-gray-50 border-t-4 border-[#E13D4B] flex items-center justify-center gap-3 hover:bg-[#E13D4B] hover:text-white transition-all group"
      >
