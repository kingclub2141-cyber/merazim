...
interface AuthContextType {
  ...
  isOwner: boolean;
  loading: boolean;
  signIn: (identifier: string, password: string) => Promise<{ data?: any, error?: any, role?: string }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchUserData = async (userId: string, email: string | undefined) => {
    try {
      ...
      if (userData) {
        ...
        // Fallback: Check auth metadata if no user record exists yet
        ...
        if (user) {
          ...
          if (metaRole) {
            ...
            setProfile({
              ...
              email: user.email,
              role: metaRole,
              member_id: user.user_metadata?.member_id,
            ...
            if (user.user_metadata?.gym_id) {
               const { data: gymData } = await supabase
                .from('gyms')
                .select('*')
                .eq('id', user.user_metadata.gym_id)
                .maybeSingle();
              if (gymData) setGym(gymData);
            }
          }
  ...
  const signIn = async (identifier: string, password: string) => {
    ...
    // Return the role from metadata as an immediate result for the login page
